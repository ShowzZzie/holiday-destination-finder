# backup version, with working SearchFlights logic. Attempting to use SearchDates in the original file now
...
def find_cheapest_trip(origin: str, destination: str, from_date: str, to_date: str, trip_length: int):
   ...
   #  best = None # best = (price, currency, stops, airlines, dep, ret)
    ...
    for helper in found_flights:
        ...
        trip_price_raw = outbound.price if outbound.price == return_flight.price else (outbound.price + return_flight.price)
        try:
            if source_currency == "EUR":
                trip_price = float(trip_price_raw)
            else:
                trip_price = float(c.convert(trip_price_raw, source_currency, "EUR"))
        except Exception:
            _ERRORS["other_err_det"] += 1
            continue
